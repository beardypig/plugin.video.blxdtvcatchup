...
def build(bld):
    ...
    for f in ('LICENSE', 'changelog.txt', 'README.md', 'resources', 'addon.py', 'icon.png'):
...
def dist(ctx):
    ...
    ctx.files = ctx.path.ant_glob("build/*.xml build/*.txt build/*.md build/LICENSE build/*.py build/*.png resources")
